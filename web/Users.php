...
class Users {
	...
	function __construct( $file = '../users.dat' ) {
		$this->userFile = $file;
		if( !( $userFileContent = file_get_contents( $file ) ) ) {
			die( 'User Data File Inaccessible' );
		}
		if( !( $userFileArray = json_decode( $userFileContent, true ) ) ){
			die( 'User Data File Corrupted' );
		}
		$this->users = $userFileArray;
	}
...
class User {
	...
	function setOTPCookie() {
		...
		// about using the user agent: It's easy to fake it, but it increases the barrier for stealing and reusing cookies nevertheless and it doesn't do any harm (except that it's invalid after a browser upgrade, but that may be even intented)
