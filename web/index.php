...
if ( $username = $users->hasSession() ){
	...
	if ( $user->isLoggedIn() ) {
		...
		// show the QR code if requested
		if ( isset( $_GET['showqr'] ) ) {
			$secret = $user->getSecret();
			include( '../tmpl/show-qr.php' );
		}
	} elseif ( $user->isOTP() && isset( $_POST['otp'] ) ) {
  ...
  die();
	...
	if ( $user && $user->auth( $_POST['password'] ) ){
		// try to authenticate the password and start the session if it's correct.
		$user->startSession();
		// check if the user has a valid OTP cookie, so we don't have to ask for the current token and can directly log in
		if ( $user->hasValidOTPCookie() ) {
			include( '../tmpl/loggedin.php' );
			$user->doLogin();
		} elseif ( !$user->getSecret() ) {
		// try to get the users' secret from the db, if they don't have one, generate one, store it and show it.
			include( '../tmpl/loggedin.php' );
			...
			// if the user neither has a valid OTP cookie nor it's the first login ask for the OTP
			$user->doOTP();
